...
- 앞으로 저장소 전체 README에서 Codex hook/session lifecycle 동작을 설명할 때는, Codex App이 같은 working directory에서 짧게 끝나는 helper model session을 추가로 띄울 수 있으므로 안정적인 watched process id가 없을 때 LidGuard가 Codex에 대해 working-directory-only watchdog fallback을 의도적으로 사용하지 않는다고 반드시 명시해야 한다.
...
- 남아 있는 모든 활성 세션이 soft-lock 상태가 되면 LidGuard는 임시 keep-awake 보호를 해제하고, 임시 lid 정책 변경을 복원한 뒤, 덮개가 닫혀 있으면 설정된 suspend 흐름을 시작해야 한다.
...
- keep-awake 보호가 적용 중이고 노트북 덮개가 닫혀 있을 때는, 선택적 Emergency Hibernation thermal monitor가 10초마다 시스템 온도를 확인하고 설정된 임계값에 도달하면 즉시 hibernate를 요청해야 한다.
...
- `LidGuardLib`
  - `net10.0` 대상 라이브러리다.
  - 공용 provider/hook 유틸리티는 일반 `*.cs` 파일에 둔다.
...
- `LidGuard.slnx`
  - `LidGuardLib.Commons`, `LidGuardLib`, `LidGuard`를 포함하는 루트 solution 파일이다.
...
- pre-suspend webhook URL이 설정되어 있으면, LidGuard는 suspend를 요청하기 전에 JSON을 POST해야 한다. body에는 `reason`이 포함되어야 하고, soft-lock 때문에 suspend되는 경우에는 soft-locked session 수까지 포함해야 한다.

### Emergency Hibernation Thermal Monitor

- Emergency Hibernation은 `SystemThermalInformation.GetSystemThermalInformation()`을 사용해 사용 가능한 시스템 thermal zone 중 가장 높은 섭씨 온도를 읽는다.
- thermal monitor는 공유 keep-awake 보호가 적용 중이고 덮개가 닫혀 있을 때만 동작한다.
...
- Emergency Hibernation은 일반 suspend mode, post-stop suspend delay, post-stop suspend sound 설정을 무시한다.
...
- parent process id가 없으면, 그 fallback이 충분히 신뢰할 수 있는 provider에 한해서만 hook working directory와 `ICommandLineProcessResolver`를 사용한다. Codex는 Codex App helper session 때문에 false cleanup이 날 수 있어 implicit fallback을 현재 사용하지 않는다.
...
- `current-temperature`는 Windows가 현재 인식하는 가장 높은 system thermal-zone 온도를 섭씨로 출력하고, thermal-zone 데이터를 구할 수 없으면 unavailable로 보고한다.
- `settings`는 기본 설정을 출력/수정하고, 실행 중인 runtime이 있으면 즉시 반영한다.
- `settings`는 `--emergency-hibernation-on-high-temperature`와 `--emergency-hibernation-temperature-celsius`도 제공하며, 임계 온도 옵션은 70부터 110까지만 허용한다.
...
- `provider_start_session`은 provider가 사용자 프롬프트 처리를 시작하기 전에 호출하도록 의도한다.
...
- 각 세션은 soft-lock 상태, 이유, 감지 시각도 함께 가진다.
...
- 새 tool 실행 같은 provider activity가 감지되면 해당 세션의 현재 soft-lock 상태를 해제한다.
- Codex 세션은 provider별 예외 경로를 가진다. Codex 세션이 이미 soft-lock 상태인 경우, LidGuard는 해당 세션 transcript JSONL 파일에서 실제 내용 증가가 5회 관찰되면 그 soft-lock을 해제할 수 있다. 이때 hook이 준 `transcript_path`를 우선 사용하고, 없으면 session id 기준으로 `~/.codex/sessions` 아래에서 유일하게 매칭되는 transcript를 fallback으로 사용한다.
...
- `AgentProvider.Codex` 세션도 explicit watched process id가 없을 때는 working directory만으로 watched process를 자동 해석하지 않는다. Codex App이 같은 working directory에서 짧게 끝나는 helper model session을 띄울 수 있어 fallback이 잘못된 프로세스에 바인딩될 수 있기 때문이다.
...
- `LidGuardSettingsMcpTools`
  ...
  - Emergency Hibernation 온도 설정을 포함해 여러 설정 필드를 한 번의 호출에서 바꿀 수 있는 `update_settings`를 노출한다.
...
- 외부 provider가 안정적인 session identifier를 제공할 수 있으면 모델에 그 값을 넘겨야 한다. 그런 값이 없으면 모델이 안정적인 식별자를 직접 만들고 세션이 정말 끝날 때까지 계속 재사용해야 한다.
...
- `provider_start_session` 설명에는 사용자 프롬프트 실행 전에 호출하라고 적어야 한다.
...
- Codex는 현재 공개 hook 표면에 대응되는 `Notification` 이벤트가 없어 notification 기반 soft-lock 감지는 아직 지원하지 않는다. 향후 Codex가 notification 또는 기계가 읽을 수 있는 pending-state hook을 노출하면 그때 지원을 추가한다.
- Codex에는 notification 스타일의 soft-lock 해제 신호도 없기 때문에, LidGuard는 `UserPromptSubmit`에서 받은 `transcript_path`를 기록하고 Codex 전용 예외 경로로 transcript JSONL 감시를 사용한다. Codex 세션이 이미 soft-lock 상태가 된 뒤 transcript 실제 내용 증가가 5회 누적되면 soft-lock을 해제한다. `transcript_path`가 비어 있으면 session id 기준으로 `~/.codex/sessions`에서 유일한 transcript 매칭을 fallback으로 사용한다.
- Codex hook input에는 안정적인 parent process id가 없다. 그래서 LidGuard는 Codex에 대해서는 explicit watched process id만 사용하고, Codex App이 같은 working directory에서 title generation 같은 짧은 helper model session을 띄울 수 있기 때문에 working-directory-only watchdog fallback은 의도적으로 건너뛴다.
...
- `PreToolUse`, `PostToolUse`, `PostToolUseFailure`는 non-`AskUserQuestion` tool에 대해 provider activity를 기록하고 해당 세션의 현재 soft-lock 상태를 해제한다.
...
- Telemetry-only event: `sessionStart`, `sessionEnd`, `errorOccurred`.
...
- `preToolUse`는 runtime을 stop하지 않고, 덮개가 닫혀 있을 때만 `ask_user`를 deny해서 사용자가 응답할 수 없는 soft lock을 막고, non-`ask_user` tool activity가 감지되면 현재 soft-lock 상태를 해제한다.
...
- `sessionStart`, `sessionEnd`, `errorOccurred`는 telemetry만 기록한다.
...
- Windows API 호출과 Windows-only 가정은 `LidGuardLib`의 `*.windows.cs` 파일에 둔다.
