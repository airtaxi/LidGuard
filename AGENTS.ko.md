...
LidGuard는 Codex, Claude Code, GitHub Copilot CLI처럼 오래 실행되는 로컬 AI 코딩 에이전트를 위한 Windows 우선 유틸리티이며 systemd/logind Linux와 macOS도 지원한다.

목표는 추적 중인 에이전트 세션 중 하나라도 아직 보호가 필요할 때 지원되는 로컬 시스템이 잠들지 않도록 유지하고, 세션이 끝나거나 suspend 가능 상태가 되면 사용자의 원래 전원 정책을 복원하는 것이다.
...
- soft-lock이 아닌 활성 세션이 하나 이상 있으면 Windows는 `PowerRequestSystemRequired`와 `PowerRequestAwayModeRequired`로 idle sleep을 막고, Linux는 systemd/logind sleep 및 idle inhibitor를 유지하며, macOS는 AC/DC 전원 모두에서 동작하는 `caffeinate -i` assertion을 유지한다.
...
- 선택 설정으로 Windows에서는 활성 전원 계획의 덮개 닫힘 동작을 임시로 `Do Nothing`으로 변경하고, Linux에서는 `handle-lid-switch` inhibitor를 유지하며, macOS에서는 `pmset -a disablesleep 1`을 임시 적용한다.
...
- `LidGuard`
  ...
  - Linux 전용 systemd/logind, `/proc`, `/sys` 구현은 `*.linux.cs`에 둔다.
  - macOS 전용 runtime/process/power 구현은 `*.macOS.cs`에 둔다.
...
- Linux 지원은 systemd/logind 환경을 대상으로 한다.
- 일반 sleep 및 idle 방지는 `systemd-inhibit` block inhibitor를 사용한다.
- `PreventSystemSleep`은 `sleep` inhibitor에 매핑된다.
- `PreventSystemSleep` 및 `PreventDisplaySleep`은 `idle` inhibitor에 매핑된다.
- `PreventAwayModeSleep`은 Windows 전용이며 Linux inhibitor 선택이나 Linux settings/help output에 영향을 주면 안 된다.
- 임시 lid-close 보호는 `ChangeLidAction`이 켜져 있을 때만 별도의 `handle-lid-switch` inhibitor에 매핑된다.
- 즉시 sleep/hibernate는 `systemctl suspend` 또는 `systemctl hibernate`를 사용한다.
- Linux runtime 동작은 장기 `sudo -v` cache에 의존하면 안 되며, 지속적인 privileged logind 동작은 Linux polkit rule command로 준비해야 한다.
...
- `PreventDisplaySleep`은 `caffeinate -d`에 매핑된다.
- LidGuard의 cross-platform `PreventAwayModeSleep` 설정 때문에 `caffeinate -s`를 사용하면 안 된다. Away-mode sleep prevention은 Windows 전용이며, macOS sleep prevention은 AC/DC 전원 모두에서 동작하도록 `caffeinate -i`에 의존해야 한다.
- 임시 lid-close 보호는 `ChangeLidAction`이 켜져 있을 때만 `pmset -a disablesleep 1`에 매핑된다.
- `SleepDisabled`를 변경하기 전에 원래 상태를 pending lid-action backup JSON 경로에 저장하고, 보호 종료 또는 다음 CLI recovery path에서 일반 명령 실행 전에 복원한다.
- 즉시 Sleep은 `pmset sleepnow`를 사용한다.
- Hibernate는 현재 지원되는 `hibernatemode`를 백업하고 `hibernatemode 25`를 쓴 뒤 `pmset sleepnow`를 요청하며, `pmset sleepnow` 반환 직후 즉시 복원하지 않고 다음 CLI recovery path가 pending hibernatemode backup을 복원하도록 남긴다. `pmset sleepnow`가 실패하면 가능한 경우 hibernatemode를 즉시 rollback해야 한다.
- macOS privileged runtime 동작은 `macos-permission install`이 허용한 정확한 명령에 대해서만 non-interactive `sudo -n`을 사용해야 하며, background runtime path에서 prompt하지 않고 설정 안내와 함께 실패해야 한다.
...
- Linux에서 `ChangeLidAction=true`는 보호가 적용되는 동안 systemd/logind `handle-lid-switch` inhibitor를 유지한다는 뜻이다. 배포판 전원 설정 파일을 수정하지 않는다.
- macOS에서 `ChangeLidAction=true`는 보호가 적용되는 동안 `pmset -a disablesleep 1`을 임시 적용하고, 원래 `SleepDisabled` 값을 pending backup state에 기록한 뒤 보호 종료 또는 crash recovery 중 복원한다는 뜻이다.
...
- Linux lid state는 `/proc/acpi/button/lid/*/state`를 읽고, 읽을 수 있는 lid state가 없으면 `Unknown`을 보고한다.
- Linux visible display monitor count는 `/sys/class/drm/*/status`를 읽고 `connected` connector를 센다. 최종 suspend 가능성 확인에서는 `LidSwitchState`가 `Closed`일 때 `eDP`, `LVDS`, `DSI` 같은 internal connector family를 제외한다.
- Linux 즉시 sleep/hibernate는 `systemctl suspend` 또는 `systemctl hibernate`를 사용한다.
- macOS lid state는 `ioreg` clamshell state를 읽고, 읽을 수 있는 state가 없으면 `Unknown`을 보고한다.
...
- Linux에서는 working-directory process resolution에 `/proc/<pid>/cwd`, `/proc/<pid>/comm`, `/proc/<pid>/cmdline`, `/proc/<pid>/stat`을 사용하고 process exit watching에는 `Process.GetProcessById().WaitForExitAsync()`를 사용한다.
- Linux에서 Codex shell-host fallback은 해석된 후보 프로세스 자신이나 직계 부모가 `bash`, `zsh`, `fish`, `sh`, `dash`, `pwsh`일 때만 허용된다.
- macOS에서는 working-directory process resolution에 `ps`와 `lsof` current-working-directory inspection을 사용하고 process exit watching에는 `Process.GetProcessById().WaitForExitAsync()`를 사용한다.
- macOS에서 Codex shell-host fallback은 해석된 후보 프로세스 자신이나 직계 부모가 `zsh`, `bash`, `fish`, `sh`, `pwsh`일 때만 허용된다.
...
- `current-lid-state`는 closed-lid 정책 판단에 쓰는 것과 같은 플랫폼 lid-state source 기준으로 현재 lid switch state를 출력한다.
...
- 세션 실행 이벤트는 플랫폼 local application data directory에 JSON lines로 기록하며 최신 500개를 유지한다. 비활동 timeout으로 인한 soft-lock 전환은 `session-timeout-softlock-recorded`로 기록된다.
- First-chance, unhandled, unobserved task exception은 inner exception 상세를 포함해 플랫폼 local application data directory의 `log/exceptions.log`에 append한다. Unobserved task exception은 이 처리 과정에서 observed 상태로 표시해야 한다.
- 최근 suspend request 기록은 플랫폼 local application data directory에 JSON lines로 기록되며, 활성화되어 있을 때 설정된 최신 entry count를 유지한다.
...
- 기본 설정은 플랫폼 local application data directory에 저장한다.

### Linux 권한 명령

- `linux-permission`은 Linux build에만 컴파일되며 Windows routing이나 help에 나타나면 안 된다.
- `linux-permission status`는 현재 사용자, managed polkit rule 상태, `systemd-inhibit` availability, `systemctl` availability, 가능한 경우 logind `CanSuspend` / `CanHibernate` capability 값을 보고한다.
- `linux-permission check`는 inhibitor acquire/release, `systemctl --version`, logind suspend/hibernate capability query를 비파괴적으로 검증한다. 실제 suspend나 hibernate를 요청하면 안 된다.
- `linux-permission install`은 root 권한 또는 one-time `sudo`를 사용해 `/etc/polkit-1/rules.d/49-lidguard.rules`를 쓰고, LidGuard에 필요한 logind action만 현재 target user에게 허용한다.
- `linux-permission remove`는 파일에 LidGuard managed marker가 있을 때만 rule을 삭제하고 unmanaged file 삭제는 거부해야 한다.

### macOS 권한 명령

- `macos-permission`은 macOS build에만 컴파일되며 Windows 또는 Linux routing/help에 나타나면 안 된다.
- `macos-permission status`는 현재 사용자, 직접 inspect 가능한 경우 managed sudoers rule 상태, privileged same-value `pmset` usability, `caffeinate`, `pmset`, `powermetrics`, `ioreg`, `system_profiler` availability, 가능한 경우 현재 `SleepDisabled`와 `hibernatemode` 값을 보고한다. Sudoers content inspect를 위해 `sudo cat`이나 managed sudoers allowlist 밖의 명령을 사용하면 안 된다.
- `macos-permission check`는 `caffeinate` acquire/release, `pmset -g`, same-value privileged `pmset disablesleep`, same-value privileged `pmset hibernatemode`, privileged `powermetrics --samplers smc` sample을 비파괴적으로 검증한다. 실제 sleep이나 hibernate를 요청하면 안 된다.
- `macos-permission install`은 root 권한 또는 one-time `sudo`를 사용해 `/private/etc/sudoers.d/lidguard`를 쓰고, 생성된 rule을 `visudo -cf`로 검증하며, LidGuard에 필요한 정확한 privileged command만 현재 target user에게 허용한다.
- managed sudoers rule은 `/usr/bin/pmset -a disablesleep 0`, `/usr/bin/pmset -a disablesleep 1`, `/usr/bin/pmset -a hibernatemode 0`, `/usr/bin/pmset -a hibernatemode 3`, `/usr/bin/pmset -a hibernatemode 25`, `/usr/bin/powermetrics --samplers smc -n 1 -i 1000`만 허용한다.
- `macos-permission remove`는 파일에 LidGuard managed marker가 있을 때만 sudoers file을 삭제하고 unmanaged file 삭제는 거부해야 한다.
- `sudo -v` prepare command를 추가하면 안 된다. sudo credential cache는 tty/timeout scope라 장기 에이전트 보호의 신뢰할 수 있는 기반이 아니다.
...
- 활성 Codex 세션에 한 번이라도 watched process가 있었는데 같은 세션의 이후 Codex start에서 더 이상 watched process를 해석하지 못하면, LidGuard는 이를 취소되었거나 watched process를 잃은 세션으로 보고 일반 provider session-end webhook을 보내지 않은 채 제거한다.
...
- 일반 idle sleep 방지: 켜짐.
- away-mode sleep 방지: Windows에서만 켜짐. Linux 및 macOS 빌드는 이 설정을 노출하지 않고 disabled로 normalize한다.
- display sleep 방지: 꺼짐.
...
- Codex hook input에는 안정적인 parent process id가 없다. 그래서 LidGuard는 Codex에 대해 explicit watched process id를 우선 사용하지만, 그 값이 없을 때도 해석된 Codex 후보 프로세스 자신이나 직계 부모가 `cmd.exe`, `pwsh.exe`, `powershell.exe` 인 shell-hosted 경우에는 working-directory fallback을 사용할 수 있다. 다만 그 cleanup 경로는 `process=none` Codex 세션을 절대 제거하지 않는다. 하지만 이미 활성 상태인 Codex 세션에 이전에 watched process가 있었고 같은 세션의 이후 Codex start에서 더 이상 watched process를 해석하지 못하면, LidGuard는 이 전환을 취소/감시 프로세스 손실 정리로 처리하며 일반 provider session-end webhook을 보내면 안 된다.
...
- 현재 수동 runtime 검증은 Windows와 Codex 중심으로만 완료되어 있다.
- Windows, Linux, macOS RID compile validation은 platform compilation regression을 잡을 수 있지만 각 target OS의 runtime behavior validation을 대체하지 않는다.
- Linux systemd/logind runtime behavior, macOS runtime behavior, Claude Code hook, GitHub Copilot CLI hook, Provider MCP flow, cross-provider concurrent-session behavior는 검증된 것으로 취급하기 전에 실제 환경 검증이 더 필요하다.

## 남은 작업

Windows, Linux, macOS CLI hook 수신 경로는 Codex, Claude Code, GitHub Copilot CLI까지 구현되어 있다. 남은 작업은 이제 lifecycle polish와 자동 회귀 검증에 더 가깝다.
