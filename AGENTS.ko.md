...
- `LidGuard`는 `start`, `stop`, `status`, `settings`, `cleanup-orphans`, `claude-hook`, `claude-hooks`, `copilot-hook`, `copilot-hooks`, `codex-hook`, `codex-hooks`, `hook-status`, `hook-install`, `hook-remove`, `hook-events`, `preview-system-sound`, `mcp-server`를 파싱한다.
...
- `hook-install`, `hook-status`, `hook-events`는 `--provider`가 생략되면 `codex`, `claude`, `copilot`, `all` 중 선택을 요청한다.
- `hook-remove`는 `--provider`가 생략되면 `codex`, `claude`, `copilot` 중 선택을 요청한다.
- `--provider all`은 구현된 세 provider를 모두 설치하거나, 상태를 확인하거나, hook event를 출력한다.
