...
- `LidGuard`는 `start`, `stop`, `status`, `settings`, `cleanup-orphans`, `claude-hook`, `claude-hooks`, `codex-hook`, `codex-hooks`, `hook-status`, `hook-install`, `hook-remove`, `hook-events`, `mcp-server`를 파싱한다.
...
- `hook-install`, `hook-status`, `hook-events`는 `--provider`가 생략되면 `codex`, `claude`, `all` 중 선택을 요청한다.
- `--provider all`은 구현된 두 provider를 모두 설치하거나, 상태를 확인하거나, hook event를 출력한다.
...
- `get_settings_status`와 `update_settings`를 노출한다.
...
- `WindowsCodexHookInstaller`
  ...
  - managed block marker가 없으면 필수 hook event 안의 유효한 `lidguard ... codex-hook` command 항목을 감지해 status와 remove에 사용한다.
...
- `PermissionRequest`는 runtime을 stop하지 않고 runtime의 덮개 상태를 조회한 뒤, 덮개가 닫혀 있을 때만 `LidGuardSettings.ClosedLidPermissionRequestDecision`에 따라 구조화된 allow/deny 결정을 반환한다.
...
- `PermissionRequest`는 runtime을 stop하지 않고 runtime의 덮개 상태를 조회한 뒤, 덮개가 닫혀 있을 때만 `LidGuardSettings.ClosedLidPermissionRequestDecision`에 따라 구조화된 allow/deny 결정을 반환한다.
- Claude Code 관련 설정, 지원, 문서 작업을 할 때는 사용자가 LidGuard와 함께 third-party prompt 성격의 hook을 쓰지 않도록 반드시 명시적이고 강하게 안내한다. LidGuard는 자신의 closed-lid `PermissionRequest` 경로에 대해서만 응답해야 하며, third-party hook prompt를 대신 처리하거나 프록시할 수 있는 것처럼 설명하거나 다루면 안 된다.
...
10. ~~Codex `SessionEnd`를 stop 처리로 매핑하고 `PermissionRequest`를 덮개 닫힘 상태에서만 설정 기반 allow/deny 결정으로 처리.~~
...
- Claude 배포나 설정을 도와줄 때는 사용자가 LidGuard와 함께 third-party prompt hook에 의존하지 않도록 반드시 명시적이고 강하게 경고해야 한다. LidGuard는 자신의 closed-lid permission decision만 수행할 수 있고, 무관한 Claude hook prompt를 대신 안전하게 응답할 수는 없다고 분명히 말해야 한다.
