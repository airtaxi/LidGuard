...
- `LidGuard.Notifications`
  ...
  - LidGuard pre-suspend webhook을 받아 구독된 브라우저로 Web Push 알림을 보낸다.
...
- Codex 세션은 transcript JSONL 감시를 provider activity source로 사용한다. transcript 길이가 증가하거나 `LastWriteTimeUtc`가 전진하면 tool event와 같은 activity 경로로 세션의 마지막 활동 시각을 갱신하고 현재 soft-lock 상태를 해제한다. 이때 hook이 준 `transcript_path`를 우선 사용하고, 없으면 session id 기준으로 `~/.codex/sessions` 아래에서 유일하게 매칭되는 transcript를 fallback으로 사용한다. 감시는 file-system 변경 알림과 짧은 metadata polling fallback을 함께 사용하므로 알림이 누락되어도 `status`가 오래된 상태로 남지 않아야 한다. 최신 transcript record가 payload type `turn_aborted`인 `event_msg`이면 LidGuard는 이를 중단된 Codex turn으로 보고 activity 기록 대신 일반 stop 경로로 세션을 정리한다.
...
notification server는 선택 사항이며 core LidGuard runtime 외부에서 동작한다. 기존 pre-suspend webhook payload를 받고, VAPID private key는 서버에만 보관해야 한다.
