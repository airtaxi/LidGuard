...
- When `closedLidStopFollowUpWebhookUrl` is valid and `postStopSuspendDelaySeconds > 0`, supported Stop hooks may keep the Stop hook open for up to that same delay while LidGuard waits for an ask-before-sleep reply; a reply cancels the pending suspend and resumes the session, while no reply falls through to the existing suspend flow without adding extra delay. If the user cancels the reply wait from the notification UI/API, reschedule the same pending suspend with a `0` second delay so `PreSuspend`, post-stop sound, final re-check, and the real suspend request start immediately.
