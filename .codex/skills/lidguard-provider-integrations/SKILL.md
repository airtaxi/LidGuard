...
- Closed-lid Stop follow-up continuation is not implemented for Provider MCP and must never be described as supported there.
...
- Only the `Stop` event may request closed-lid Stop follow-up continuation. `SessionEnd` remains a plain stop path. When Codex provides `stop_hook_active = true`, LidGuard must skip Stop follow-up continuation to avoid trapping a turn that is already continuing.
...
- Only Claude `Stop` may request closed-lid Stop follow-up continuation. `StopFailure` and `SessionEnd` must not do so. When Claude provides `stop_hook_active = true`, LidGuard must skip Stop follow-up continuation to avoid blocking on a continuation that is already in progress.
...
- GitHub Copilot CLI closed-lid Stop follow-up continuation is allowed only for `agentStop` and the VS Code-compatible `Stop` alias. `sessionEnd` must remain a plain stop path.
