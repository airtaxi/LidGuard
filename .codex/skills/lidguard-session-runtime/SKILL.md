...
- Claude hook `Stop` handling also reads recent transcript JSONL to reconcile background `Bash`/`PowerShell`/`Agent`/legacy `Task`/`Monitor` work and `TaskCreated`/`TaskCompleted` work, using delivered `task_notification` completion records, `UserPromptSubmit` `<task-notification>` payloads, and `TaskStop` tool use. Queued Claude `queue-operation` task-notification content is not a delivered completion signal. When work remains, the runtime must keep the session active instead of restoring protection, scheduling suspend, sending `PreSuspend`, sending `PostSessionEnd`, or starting runtime cleanup; only a later Claude `Stop` after tracked work is clear may enter normal stop behavior.
...
- GitHub Copilot CLI hook `agentStop` / `sessionEnd` handling also reads recent session-state JSONL to reconcile background `task` agents and async/detached `bash` / `powershell` shell work, using `read_agent` terminal states, `subagentStart` / `subagentStop`, and completion notifications such as `shell_completed`, `shell_detached_completed`, `agent_completed`, and `agent_idle`. Completion signals clear matching tracked work but do not send a final stop by themselves. When work remains, the runtime must keep the session active instead of restoring protection, scheduling suspend, sending `PreSuspend`, sending `PostSessionEnd`, or starting runtime cleanup; only a later GitHub Copilot CLI `agentStop` or `sessionEnd` after tracked work is clear may enter normal stop behavior.
