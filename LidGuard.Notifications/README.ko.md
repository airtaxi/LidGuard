...
`LidGuard.Notifications`는 LidGuard pre-suspend 및 post-session-end webhook을 받아 구독된 브라우저로 Web Push 알림을 전달하는 작은 ASP.NET Core Razor Pages 서버입니다. 브라우저 구독, webhook event, delivery 결과는 SQLite에 저장합니다.
