...
public sealed class SystemAudioVolumeController : ISystemAudioVolumeController
{
    ...
    public LidGuardOperationResult<SystemAudioVolumeState> CaptureDefaultRenderDeviceState()
    {
        ...
        var volumeResult = MacOSCommandRunner.Run(osascriptPath, ["-e", "output volume of (get volume settings)"], s_osascriptTimeout);
        ...
        var muteResult = MacOSCommandRunner.Run(osascriptPath, ["-e", "output muted of (get volume settings)"], s_osascriptTimeout);
        ...
        return LidGuardOperationResult<SystemAudioVolumeState>.Success(new SystemAudioVolumeState { MasterVolumeScalar = Math.Clamp(volumePercent, 0, 100) / 100.0f, IsMuted = isMuted });
    }
    ...
    public LidGuardOperationResult ApplyDefaultRenderDeviceVolumeOverride(int volumeOverridePercent)
    {
        ...
        var commandResult = MacOSCommandRunner.Run(osascriptPath, ["-e", $"set volume output volume {volumeOverridePercent} without output muted"], s_osascriptTimeout);
    ...
    public LidGuardOperationResult RestoreDefaultRenderDeviceState(SystemAudioVolumeState state)
    {
        ...
        var volumeResult = MacOSCommandRunner.Run(osascriptPath, ["-e", $"set volume output volume {volumePercent}"], s_osascriptTimeout);
        var muteResult = MacOSCommandRunner.Run(osascriptPath, ["-e", $"set volume output muted {muteValue}"], s_osascriptTimeout);
        return CombineVolumeChangeResults(volumeResult.Succeeded ? LidGuardOperationResult.Success() : LidGuardOperationResult.Failure(volumeResult.CreateFailureMessage("osascript set volume"), volumeResult.ExitCode), muteResult.Succeeded ? LidGuardOperationResult.Success() : LidGuardOperationResult.Failure(muteResult.CreateFailureMessage("osascript set muted"), muteResult.ExitCode));
    }
