...
internal static class LidGuardSettingsInteractiveFactory
{
    public static bool TryCreateSettings(LidGuardSettings currentSettings, out LidGuardSettings settings, out string message)
    {
        ...
        if (!LidGuardSettingsInteractivePromptReader.TryReadServerRuntimeCleanupDelayMinutesSetting(LocalizationService.GetString("SettingsNameServerRuntimeCleanupDelayMinutes"), normalizedStoredSettings.ServerRuntimeCleanupDelayMinutes, defaultSettings.ServerRuntimeCleanupDelayMinutes, out var serverRuntimeCleanupDelayMinutes, out message))
            return false;
        if (!LidGuardSettingsInteractivePromptReader.TryReadBooleanSetting(LocalizationService.GetString("SettingsNameEmergencyHibernationOnHighTemperature"), normalizedStoredSettings.EmergencyHibernationOnHighTemperature, defaultSettings.EmergencyHibernationOnHighTemperature, out var emergencyHibernationOnHighTemperature, out message))
            return false;
        if (!LidGuardSettingsInteractivePromptReader.TryReadEmergencyHibernationTemperatureModeSetting(LocalizationService.GetString("SettingsNameEmergencyHibernationTemperatureMode"), normalizedStoredSettings.EmergencyHibernationTemperatureMode, defaultSettings.EmergencyHibernationTemperatureMode, out var emergencyHibernationTemperatureMode, out message))
            return false;
        if (!LidGuardSettingsInteractivePromptReader.TryReadEmergencyHibernationTemperatureCelsiusSetting(LocalizationService.GetString("SettingsNameEmergencyHibernationTemperatureCelsius"), normalizedStoredSettings.EmergencyHibernationTemperatureCelsius, defaultSettings.EmergencyHibernationTemperatureCelsius, out var emergencyHibernationTemperatureCelsius, out message))
            return false;
        ...
        if (!LidGuardSettingsInteractivePromptReader.TryReadNonNegativeIntegerSetting(LocalizationService.GetString("SettingsNamePostStopSuspendDelaySeconds"), normalizedStoredSettings.PostStopSuspendDelaySeconds, defaultSettings.PostStopSuspendDelaySeconds, out var postStopSuspendDelaySeconds, out message))
            return false;
        if (!LidGuardSettingsInteractivePromptReader.TryReadPostStopSuspendSoundSetting(LocalizationService.GetString("SettingsNamePostStopSuspendSound"), normalizedStoredSettings.PostStopSuspendSound, defaultSettings.PostStopSuspendSound, out var postStopSuspendSound, out message)) return false;
        if (!LidGuardSettingsInteractivePromptReader.TryReadPostStopSuspendSoundVolumeOverridePercentSetting(LocalizationService.GetString("SettingsNamePostStopSuspendSoundVolumeOverridePercent"), normalizedStoredSettings.PostStopSuspendSoundVolumeOverridePercent, defaultSettings.PostStopSuspendSoundVolumeOverridePercent, out var postStopSuspendSoundVolumeOverridePercent, out message))
            return false;
        if (!LidGuardSettingsInteractivePromptReader.TryReadSuspendHistoryEntryCountSetting(LocalizationService.GetString("SettingsNameSuspendHistoryEntryCount"), normalizedStoredSettings.SuspendHistoryEntryCount, defaultSettings.SuspendHistoryEntryCount, out var suspendHistoryEntryCount, out message)) return false;
        ...
        if (!LidGuardSettingsInteractivePromptReader.TryReadBooleanSetting(LocalizationService.GetString("SettingsNameRepeatClosedLidStopFollowUp"), normalizedStoredSettings.RepeatClosedLidStopFollowUp, defaultSettings.RepeatClosedLidStopFollowUp, out var repeatClosedLidStopFollowUp, out message)) return false;
        if (!LidGuardSettingsInteractivePromptReader.TryReadUserInterfaceCultureSetting(LocalizationService.GetString("SettingsNameUserInterfaceCulture"), normalizedStoredSettings.UserInterfaceCulture, defaultSettings.UserInterfaceCulture, out var userInterfaceCulture, out message)) return false;
