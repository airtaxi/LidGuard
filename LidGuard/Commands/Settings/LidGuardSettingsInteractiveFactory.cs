...
internal static class LidGuardSettingsInteractiveFactory
{
    public static bool TryCreateSettings(LidGuardSettings currentSettings, out LidGuardSettings settings, out string message)
    {
        ...
        if (!LidGuardSettingsInteractivePromptReader.TryReadSessionTimeoutMinutesSetting(LocalizationService.GetString("SettingsNameSessionTimeoutMinutes"), normalizedStoredSettings.SessionTimeoutMinutes, defaultSettings.SessionTimeoutMinutes, out var sessionTimeoutMinutes, out message))
            return false;
        ...
        if (!LidGuardSettingsInteractivePromptReader.TryReadNonNegativeIntegerSetting(LocalizationService.GetString("SettingsNamePostStopSuspendDelaySeconds"), normalizedStoredSettings.PostStopSuspendDelaySeconds, defaultSettings.PostStopSuspendDelaySeconds, out var postStopSuspendDelaySeconds, out message))
            return false;
        if (!LidGuardSettingsInteractivePromptReader.TryReadPostStopSuspendSoundSetting(LocalizationService.GetString("SettingsNamePostStopSuspendSound"), normalizedStoredSettings.PostStopSuspendSound, defaultSettings.PostStopSuspendSound, out var postStopSuspendSound, out message))
            return false;
