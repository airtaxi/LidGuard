...
internal static class SettingsHelpContent
{
    ...
    private static string CreateSynopsis(string commandDisplayName)
    {
        ...
        return $"{commandDisplayName} settings [--reset <bool>] [--change-lid-action <bool>] {powerRequestOptions} [--prevent-display-sleep <bool>] [--watch-parent-process <bool>] [--session-timeout-minutes off|<minutes>] [--server-runtime-cleanup-delay-minutes off|<minutes>] [--emergency-hibernation-on-high-temperature <bool>] [--emergency-hibernation-temperature-mode low|average|high] [--emergency-hibernation-temperature-celsius <number>] [--suspend-mode sleep|hibernate] [--post-stop-suspend-delay-seconds <number>] [--post-stop-suspend-sound off|<system-sound>|<wav-path>] [--post-stop-suspend-sound-volume-override-percent off|<1-100>] [--suspend-history-count off|<count>] [--pre-suspend-webhook-url <http-or-https-url>] [--post-session-end-webhook-url <http-or-https-url>] [--closed-lid-permission-request-decision deny|allow] [--power-request-reason <text>]";
    }
    ...
    private static IReadOnlyList<LidGuardHelpOption> CreateOptions(string supportedPostStopSuspendSystemSounds)
    {
        ...
        {
            new("--reset <bool>", "When true, start from headless runtime defaults before applying the other supplied options."),
            new("--change-lid-action <bool>", "Toggle the temporary active power plan lid close action override."),
            new("--prevent-system-sleep <bool>", "Toggle normal system sleep prevention.")
        };
#if !LIDGUARD_LINUX && !LIDGUARD_MACOS
        options.Add(new LidGuardHelpOption("--prevent-away-mode-sleep <bool>", "Toggle PowerRequestAwayModeRequired handling."));
#endif
        options.AddRange([
            new LidGuardHelpOption("--prevent-display-sleep <bool>", "Toggle display sleep prevention."),
