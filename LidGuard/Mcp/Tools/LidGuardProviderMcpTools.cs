...
[McpServerToolType]
public sealed class LidGuardProviderMcpTools(ProviderMcpServerConfiguration providerMcpServerConfiguration, LidGuardControlService controlService)
{
    [McpServerTool(
    ...
    public async Task<LidGuardSessionCommandToolResponse> StartSession([Description("Optional working directory for the current task. Pass it when the provider can expose the active project folder.")] string workingDirectory = null, CancellationToken cancellationToken = default)
    {
        var sessionIdentifier = CreateGeneratedSessionIdentifier();
        var result = await controlService.StartSessionAsync(sessionIdentifier, AgentProvider.Mcp, providerMcpServerConfiguration.ProviderName, workingDirectory ?? string.Empty, 0, cancellationToken);
    ...
    public async Task<LidGuardSessionCommandToolResponse> StopSession([Description("The exact session identifier previously returned by provider_start_session for the ongoing session that is now truly complete.")] string sessionIdentifier, CancellationToken cancellationToken = default)
    {
        var result = await controlService.StopSessionAsync(sessionIdentifier, AgentProvider.Mcp, providerMcpServerConfiguration.ProviderName, true, "provider_stop_session", cancellationToken);
        if (!result.Succeeded) throw new McpException(result.Message);
    ...
    public async Task<LidGuardSessionCommandToolResponse> SetSoftLock([Description("The exact session identifier previously returned by provider_start_session for the session that should stay tracked but become suspend-eligible.")] string sessionIdentifier, [Description("Why autonomous work is blocked on user input, such as waiting_for_user_input, waiting_for_clarification, waiting_for_approval, waiting_for_credentials, or waiting_for_manual_step.")] string reason, CancellationToken cancellationToken = default)
    {
        var result = await controlService.SetSessionSoftLockAsync(sessionIdentifier, AgentProvider.Mcp, providerMcpServerConfiguration.ProviderName, reason, cancellationToken);
        if (!result.Succeeded) throw new McpException(result.Message);
    ...
    public async Task<LidGuardSessionCommandToolResponse> ClearSoftLock([Description("The exact session identifier previously returned by provider_start_session for the session whose earlier soft lock should be cleared.")] string sessionIdentifier, [Description("Optional reason describing why the session can resume work, such as resumed_after_user_reply.")] string reason = null, CancellationToken cancellationToken = default)
    {
        var result = await controlService.ClearSessionSoftLockAsync(sessionIdentifier, AgentProvider.Mcp, providerMcpServerConfiguration.ProviderName, reason, cancellationToken);
        if (!result.Succeeded) throw new McpException(result.Message);
    ...
    private static LidGuardSessionCommandToolResponse CreateSessionCommandToolResponse(LidGuardSessionCommandOutcome outcome)
    {
        ...
        var reuseSummary = outcome.RequestedCommand == LidGuardPipeCommands.Start ? $" Reuse session id '{outcome.RequestedSessionIdentifier}' verbatim with provider_set_soft_lock, provider_clear_soft_lock, and provider_stop_session until the work is truly complete." : string.Empty;
        var summary = outcome.Snapshot.RuntimeReachable ? $"{outcome.RuntimeMessage} Runtime now tracks {outcome.Snapshot.ActiveSessionCount} active session(s) after handling {scope}.{reuseSummary}" : outcome.Snapshot.RuntimeUnavailable ? $"{outcome.RuntimeMessage} Runtime is not running after handling {scope}.{reuseSummary}" : $"{outcome.RuntimeMessage} Runtime status after handling {scope} is unavailable: {outcome.Snapshot.RuntimeMessage}.{reuseSummary}";
