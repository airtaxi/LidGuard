...
{
    public static MacOSCommandResult Run(string fileName, IEnumerable<string> arguments, TimeSpan timeout = default)
    {
        if (timeout <= TimeSpan.Zero)
        {
            try { return RunAsync(fileName, arguments, CancellationToken.None).GetAwaiter().GetResult(); }
            catch (OperationCanceledException) { return MacOSCommandResult.Failure($"Command was canceled: {fileName}"); }
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using (process)
        {
            ...
            return MacOSCommandResult.Success(process.ExitCode, standardOutput, standardError);
        }
    }
