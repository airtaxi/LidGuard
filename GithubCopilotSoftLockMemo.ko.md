...
이 문서는 GHCP hook 지원 구현 전에 보는 참고 문서다.
...
### 2. `ask_user` via `preToolUse` [guard 가능]
...
### 4. `notification(permission_prompt|elicitation_dialog)` [감지 가능]
...
1. `userPromptSubmitted`를 start event로 파싱한다.
2. `agentStop`을 stop event로 파싱한다.
3. `permissionRequest`에서 closed-lid allow/deny를 구현한다.
4. `preToolUse(toolName=ask_user)`에서 closed-lid deny를 구현한다.
5. `notification(permission_prompt|elicitation_dialog)`를 event log에 기록한다.
6. `hook-install`과 `hook-status`에서 non-LidGuard `agentStop` hook 존재를 경고한다.
7. GHCP hook input에서 parent process id를 직접 받을 수 있는지 확인하고, 없으면 working directory 기반 resolver fallback을 적용한다.
...
- `notification(permission_prompt|elicitation_dialog)`는 감지 전용 경로다.
