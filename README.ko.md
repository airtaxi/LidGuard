...
OpenCode는 전역 플러그인으로 연결됩니다. OpenCode가 세션에 더 할 일이 없다고 알려오면 LidGuard가 답장을 기다릴 수 있습니다. 답장하면 같은 OpenCode 채팅에 그 내용을 다시 보내 작업을 이어갑니다. OpenCode는 마무리 단계를 LidGuard가 붙잡아 둘 수 없으므로, 답장이 도착할 때 OpenCode가 아직 실행 중이고 해당 채팅이 남아 있어야 합니다. 같은 플러그인이 OpenCode 1과 OpenCode 2를 모두 지원합니다. OpenCode를 업그레이드한 뒤에는 `lidguard hook-install --provider opencode`를 한 번 실행해 플러그인 파일을 새로 받아 주세요.
...
- 현재 상태 표시: 보호 중인 세션 수, 연결된 도구와 세션 식별값, 감시 중인 프로세스 번호, 사용자 입력 대기 여부, provider 백그라운드 작업 대기 여부, 작업 폴더, 시작 시각, 마지막 활동 시각, 덮개 상태, 보이는 모니터 수, 실시간 터미널 상태 화면을 확인할 수 있습니다.
...
- 작업 감시와 정리: AI 도구의 부모 프로세스를 감시하고 이미 끝난 세션을 정리하며 오래 활동이 없는 세션은 절전 방지를 풀 수 있습니다. provider 백그라운드 작업 때문에 종료를 유예 중인 세션도 같은 시간 제한을 받으므로 종료나 완료 알림이 유실되어도 덮개 닫힘 동작 변경은 설정 시간이 지나면 원래대로 되돌아갑니다. 모든 세션이 끝난 뒤 LidGuard가 얼마 뒤 자동 종료될지도 정할 수 있습니다.
