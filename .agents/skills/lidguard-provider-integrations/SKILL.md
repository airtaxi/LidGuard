...
- The managed plugin is a single default export that supports both OpenCode 1 and OpenCode 2. OpenCode 1 calls the object's `server()` implementation, and OpenCode 2 reads the object's `id` and `setup()` implementation. The managed plugin version marker is `// LidGuard OpenCode plugin version: 2`, and `hook-status` treats files with an older marker as needing update.
- OpenCode 2 does not run OpenCode 1 plugin functions or `server()` implementations, and it requires the default export to be an object with `id` and `setup`. Keep both implementations in the same managed file so one plugin covers both generations.
- OpenCode 2 discovers direct `.js` files under the global config directory `plugins` folder exactly like OpenCode 1, so the managed plugin path is unchanged. OpenCode 2 loads the plugin once per location while `ctx.event.subscribe()` streams events for every location, so the `setup()` implementation filters events by `event.location.directory === ctx.location.directory` and ignores events without a location.
- OpenCode 1 start event: `chat.message`. OpenCode 2 start hook: `ctx.session.hook("prompt")`, which the plugin forwards to the same internal `chat.message` event.
- OpenCode 1 permission decision event: `permission.ask`. OpenCode 2 permission hook: `ctx.permission.hook("evaluate")`, which the plugin forwards to the same internal `permission.ask` event and maps the returned `status` to `event.effect` plus the returned `message` to `event.message`.
- OpenCode 1 activity events: `tool.execute.before`, `tool.execute.after`, `permission.replied`, `question.replied`, `question.rejected`, `question.v2.replied`, and `question.v2.rejected`. OpenCode 2 activity hooks and events: `ctx.tool.hook("execute.before")`, `ctx.tool.hook("execute.after")`, `permission.replied`, `form.replied`, and `form.cancelled`.
- OpenCode 1 soft-lock events: `permission.asked`, `question.asked`, and `question.v2.asked`. OpenCode 2 soft-lock events: `permission.asked` and `form.created`.
- OpenCode 1 stop trigger events (tracked by plugin): `session.idle`, `session.deleted`, and `session.error`; `session.status` is intentionally excluded so only `session.idle` drives stop and continuation behavior. OpenCode 2 stop trigger events: `session.execution.succeeded`, `session.execution.failed`, `session.execution.interrupted`, and `session.deleted`; OpenCode 2 does not emit `session.idle` or `session.status`.
- OpenCode 2 `session.execution.succeeded` is the normal idle end and the only OpenCode 2 event that may request ask-before-sleep reply continuation. `session.execution.failed`, `session.execution.interrupted` (except the `shutdown` reason, which is ignored because the execution resumes after a restart), and `session.deleted` remain plain stop paths.
- OpenCode 1 `message.part.updated` and OpenCode 2 `session.text.ended` are tracked internally by the managed plugin to cache the last visible assistant text (`part.type === "text"` only for OpenCode 1); they do not invoke the runtime activity hook. OpenCode 2 reasoning and tool output are ignored for this cache.
...
- For `chat.message`, it sends internal `start --provider opencode` for both the OpenCode 1 `chat.message` hook and the OpenCode 2 `ctx.session.hook("prompt")` hook.
- For `permission.ask`, it does not stop the runtime; it queries the runtime lid state and visible display monitor count. When the lid is closed and the visible display monitor count is `0`, Deny and Allow return OpenCode-specific structured JSON with `status` of `deny` or `allow`; Ask marks the session soft-locked with reason `closed_lid_permission_request_ask` and returns empty stdout so OpenCode's normal permission flow continues. The OpenCode 1 plugin contract only exposes `output.status`, so the V1 implementation ignores the `message` field; the OpenCode 2 permission evaluate hook exposes `effect` and `message`, so the `setup()` implementation forwards the deny message into `event.message`.
- For activity events, it records provider activity and clears the current session soft-lock state.
- For soft-lock events, it marks the session soft-locked with the event name as the reason. OpenCode 2 `form.created` with the MCP elicitation sentinel session id `global` is ignored because it cannot be attached to a tracked session.
- For stop trigger events, it sends internal `stop --provider opencode`. `session.idle` (OpenCode 1) and `session.execution.succeeded` (OpenCode 2) are treated as normal provider session ends.
- Only `session.idle` (OpenCode 1) and `session.execution.succeeded` (OpenCode 2) may request ask-before-sleep reply continuation. `session.deleted`, `session.error`, `session.execution.failed`, and `session.execution.interrupted` remain plain stop paths and must not reinject prompts.
- For OpenCode 1 ask-before-sleep reply continuation, the managed plugin consumes the hook stdout `StopHookContinuationDecisionOutput` (`decision = block`, `reason = <reply>`) and calls `client.session.prompt({ path: { id: sessionID }, body: { parts: [{ type: "text", text: reply }] } })` to send the reply back into the same OpenCode session. For OpenCode 2 it calls `ctx.session.prompt({ sessionID, text: reply })`. Do not set `noReply`; the model should answer normally.
- OpenCode `stopHookActive` is simulated by plugin-local per-session state after a successful continuation prompt. The next `session.idle` (OpenCode 1) or `session.execution.succeeded` (OpenCode 2) payload includes `stopHookActive = true` so LidGuard can apply `RepeatClosedLidStopFollowUp` consistently with providers that expose a real active Stop hook flag.
- OpenCode 2 `ctx.event.subscribe()` is a volatile stream that can end on subscriber overflow or a dropped connection, so the managed plugin re-subscribes after a short delay until the plugin is unloaded.
...
- OpenCode plugin payloads do not provide a stable parent process id, so LidGuard resolves the watched process from hook process ancestry when `WatchParentProcess` is enabled, accepting OpenCode CLI, the OpenCode 2 background service, and bun/node/npm/npx wrapper processes. Working directory remains metadata only.
