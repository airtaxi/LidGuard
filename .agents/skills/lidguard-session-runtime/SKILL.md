...
- OpenCode differs from blocking-capable Stop hooks: the OpenCode 1 plugin waits after `session.idle` and the OpenCode 2 plugin waits after `session.execution.succeeded`, consumes the continuation stdout, and sends the reply back into the same session with `client.session.prompt()` on OpenCode 1 or `ctx.session.prompt({ sessionID, text })` on OpenCode 2. This simulates the same user experience but depends on OpenCode still having the session available.
