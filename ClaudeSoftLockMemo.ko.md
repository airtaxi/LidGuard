...
2026-04-29 기준 LidGuard 저장소에는 Claude `Notification` 기반 softlock 감지와 runtime-led softlock orchestration이 반영되어 있다.
...
### 2. Notification hook 기반 softlock telemetry 추가 [구현됨]

`PermissionRequest`와 `Elicitation`이 직접 guard를 맡는 구조는 그대로 두고, `Notification` hook은 감지 전용 telemetry로 추가하는 편이 좋다.

구현 상태:

- [완료] Claude installer/status/snippet에 `Notification` hook 확장을 반영했다.
- [완료] 기본 matcher를 `permission_prompt|elicitation_dialog|elicitation_complete|elicitation_response`로 넣었다.
- [완료] lid closed 상태의 `permission_prompt`와 `elicitation_dialog`는 session soft-lock state로 기록한다.
- [완료] `elicitation_complete`와 `elicitation_response`는 현재 soft-lock 해제 신호로 사용한다.
- [완료] hook output은 blocking/decision이 아니라 로그/진단용 no-op 원칙을 유지한다.
- [완료] `PreToolUse`, `PostToolUse`, `PostToolUseFailure` activity를 감지하면 현재 soft-lock 상태를 해제한다.
- [완료] 남아 있는 active session이 전부 soft-lock이면 runtime이 protection을 해제하고 닫힌 lid 기준 suspend 흐름을 시작한다.
- [후속 후보] `elicitation_url_dialog`와 `worker_permission_prompt`는 최신 build 실측 후 확장 여부를 결정한다.
